...
const STORAGE_KEY = 'solvency_state_v1';
        ...
        ${weak.map(w => `
          <div class="flex items-center justify-between p-2 rounded-lg bg-slate-50 dark:bg-slate-700/50">
            <div class="flex-1 min-w-0">
              <div class="text-sm font-medium truncate">第${w.no}号 ${escHtml(w.title)}</div>
      ...
      ${RULES.map(r => {
        ...
        return `
          <div class="card !mb-0 cursor-pointer hover:shadow-md transition" data-action="rule" data-rule="${r.no}">
            ...
            <div class="text-sm font-semibold leading-snug mb-2">第${r.no}号 ${escHtml(r.title)}</div>
    ...
    <div class="card">
      ${wrongQs.slice(0, 100).map(q => {
        ...
        return `
          <div class="qrow">
            <div class="flex-1 min-w-0">
              <div class="text-xs text-slate-500 mb-0.5">第${q.rule_no}号 · ${q.qtype==='single'?'单选':'多选'}</div>
...
function showQ() {
  ...
  $('#quiz-rule-tag').textContent = `第${q.rule_no}号 · ${escHtml(q.rule_title)} · ${q.qtype==='single'?'单选':'多选'} · 难度${'★'.repeat(q.difficulty)}`;
...
function finishSession(timeout) {
  ...
  const detail = wrongList.length > 0
    ? `<div class="mb-1 font-semibold text-slate-700 dark:text-slate-200">错题已自动加入错题集：</div>` + wrongList.slice(0,8).map(r => {
        const q = findQ(r.qid);
        if (!q) return '';
        return `<div class="text-xs">· 第${q.rule_no}号 ${escHtml(q.q.slice(0,30))}${q.q.length>30?'…':''} (答${r.picked}/正${Array.isArray(r.correctAns)?r.correctAns.join(''):r.correctAns})</div>`;
...
function startPractice({ ruleNo, random }) {
  ...
  newSession(qs, { title: ruleNo ? `第${ruleNo}号练习` : '随机练习', mode: 'practice', adaptive: true });
}
...
function exportData() {
  ...
  a.download = `偿二代学习进度-${todayStr()}.json`;
  a.click();
  toast('已导出');
}
function importData() {
